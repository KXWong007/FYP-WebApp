...
@section('content')
    ...
    <div class="orderitems-info mb-3 d-flex justify-content-between align-items-center">
        <span><strong>Order ID:</strong> {{ $orderDetails->orderId ?? 'N/A' }}</span> 
        <span><strong>Customer ID:</strong> {{ $orderDetails->customerId ?? 'N/A' }}</span> 
        <span><strong>Order Date:</strong> {{ $orderDetails->orderDate ?? 'N/A' }}</span> 
        <span><strong>Table Number:</strong> {{ $orderDetails->tableNum ?? 'N/A' }}</span>
        <span><strong>Total Amount: </strong>RM {{ $orderDetails->totalAmount ?? 'N/A' }}</span>
        <span><strong>Status:</strong> {{ $orderDetails->orderStatus ?? 'N/A' }}</span>
    </div>
...
@section('scripts')
    ...
    <script>
        ...
        window.orderId = "{{ $orderDetails->orderId }}";  
    </script>
    <script src="{{ asset('js/orderitems.js') }}"></script>
@endsection
