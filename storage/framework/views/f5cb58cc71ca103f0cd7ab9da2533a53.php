...
<div class="container">
    ...
    <form method="GET" action="<?php echo e(route('forecast.index')); ?>">
        <div class="row mb-3">
            <div class="col">
                ...
                <select class="form-select" id="duration" name="duration" onchange="this.form.submit()">
                    <option value="7" <?php echo e($duration == 7 ? 'selected' : ''); ?>>1 Week (7 days)</option>
                    <option value="30" <?php echo e($duration == 30 ? 'selected' : ''); ?>>1 Month (30 days)</option>
                </select>
            </div>
        </div>
    </form>
    ...
    <ul class="list-group">
        <?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <li class="list-group-item" onclick="toggleDetails('<?php echo e($item->inventoryId); ?>', event)">
                <h5><?php echo e($item->itemName); ?> (<?php echo e($item->measurementUnit); ?>)</h5>

                <!-- Button to Trigger Add Usage Data Modal -->
                <button class="btn btn-primary" onclick="openAddUsageModal('<?php echo e($item->inventoryId); ?>', '<?php echo e($item->itemName); ?>')">Add Usage Data</button>

                <!-- Hidden section for graph and data -->
                <div id="details-<?php echo e($item->inventoryId); ?>" style="display: none;">
                    <canvas id="forecastChart-<?php echo e($item->inventoryId); ?>"></canvas>
                    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
                    <script>
                        const ctx<?php echo e($item->inventoryId); ?> = document.getElementById('forecastChart-<?php echo e($item->inventoryId); ?>').getContext('2d');
                        const data<?php echo e($item->inventoryId); ?> = <?php echo json_encode($item->forecasting, 15, 512) ?>;

                        new Chart(ctx<?php echo e($item->inventoryId); ?>, {
                            type: 'line',
                            data: {
                                labels: data<?php echo e($item->inventoryId); ?>.map(item => item.date),
                                datasets: [{
                                    label: 'Daily Usage',
                                    data: data<?php echo e($item->inventoryId); ?>.map(item => item.dailyUsage),
                                    ...
                                    tooltip: {
                                        ...
                                        callbacks: {
                                            label: function(context) {
                                                ...
                                                label += context.raw + ' <?php echo e($item->measurementUnit); ?>';
                                                return label;
                                            }
                                        }
                                    },
                    ...
                    <p><strong>Next <?php echo e($duration == 7 ? "Week's" : "Month's"); ?> Stock Requirement:</strong>
                        <?php
                        ...
                        ?>
                        <?php echo e(number_format($totalRequirement, 2)); ?> <?php echo e($item->measurementUnit); ?>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <?php $__currentLoopData = $item->forecasting; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $forecast): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <tr>
                                        <td><?php echo e($forecast->date); ?></td>
                                        <td><?php echo e($forecast->dailyUsage); ?> <?php echo e($forecast->measurementUnit); ?></td>
                                        <td>
                                            <!-- Edit Button to Trigger Modal -->
                                            <button class="btn btn-warning btn-sm" onclick="openEditModal(<?php echo e($forecast->id); ?>, '<?php echo e($forecast->dailyUsage); ?>', '<?php echo e($forecast->date); ?>')">Edit</button>
                                            
                                            <!-- Delete Button -->
                                            <form action="<?php echo e(route('forecast.destroy', $forecast->id)); ?>" method="POST" style="display:inline;" onclick="event.stopPropagation()">
                                                <?php echo csrf_field(); ?>
...
<script>
    ...
    function showLowStockAlerts() {
        const lowStockAlerts = <?php echo json_encode(session('low_stock_alerts', []), 512) ?>; // Pass session data to JS
        ...
        if (lowStockAlerts.length > 0) {
            ...
            <?php session()->forget('low_stock_alerts'); ?>
        }
    }
...
<?php echo $__env->make('layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\LATESTF&B\finalfyp\fyptest-Integrate\resources\views/forecasting/index.blade.php ENDPATH**/ ?>